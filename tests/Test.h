...
namespace Test {
    ...
    template<typename T>
    rr_uint difference(const char* name, const heap_darray<T>& A, const heap_darray<T>& B, size_t ntotal) {
        rr_uint err_count = 0;
    ...
    template<typename T>
    rr_uint difference(const char* name, const heap_darray_md<T>& A, const heap_darray_md<T>& B,
        size_t ntotal, const heap_darray<rr_uint>& neighbours_count)
    {
        rr_uint err_count = 0;
        for (size_t j = 0; j < ntotal; ++j) {
