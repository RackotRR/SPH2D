...
void artificial_viscosity(
	...
	const heap_darray_md<rr_floatn>& dwdr, // precomputed kernel derivative
	heap_darray<rr_floatn>& a, // out, acceleration with respect to x, y, z
	...
	heap_darray<rr_float>& art_visc_mu) // out, mu = max(hsml v_ij * r_ij / (r_ij^2 + hsml^2 etq^2))
{
	if (params.dim == 2) {
		...
		artificial_viscosity(r, v, rho, neighbours, dwdr, a, art_visc_mu);
	}
